...
if snaps:
    ...
    for c, (sym, d) in zip(cols, snaps.items()):
        delta = d["chg_pct"]
        c.metric(d["label"], f"{d['price']:,.2f}", f"{delta:+.2f}%")
else:
...
gold_now, gold_avg = gold_y["tonnes"].iloc[-1], gold_y["tonnes"].iloc[-5:].mean()
...
cards = [
    ...
    ("4", "⚡ Energy Settlement", "Active non-USD corridors",
     f"{energy_active} corridors", chip("hot", "EXPANDING") if energy_active >= 3 else chip("ok", "EARLY")),
    ...
    ("6", "📜 Bond Markets", "Foreign official Treasury custody",
     _bond if (_bond := (lambda d: "n/a — add FRED key" if d is None else f"${d['value'].iloc[-1]:,.0f}B")(fred_series("WRESCRTREAS"))) else "",
     chip("neg", "FISCAL STRESS")),
    ("7", "🏛️ Institutional Stack", "Custody / ETF / tokenization",
     f"{(inst['custody'].str.contains('Yes')).sum()} custody · {(inst['btc_etf'].str.contains('ETF|FBTC|IBIT|EZBC|SPLG')).sum()} ETF",
     chip("hot", "INFRA LIVE")),
]

for row in [cards[0:3], cards[3:7]]:
    cols = st.columns(len(row))
    for c, (num, title, sub, value, chiphtml) in zip(cols, row):
        with c:
            st.markdown(f"""
            <div style="background:#14161c;border:1px solid #262a33;border-radius:12px;padding:16px;height:100%">
              ...
              <div style="margin-top:8px">{chiphtml}</div>
            </div>""", unsafe_allow_html=True)

st.divider()

# ---------- Composite Reserve Diversification Index ----------
def norm(x, lo, hi): return max(0.0, min(1.0, (x - lo) / (hi - lo)))
...
for t, topic in zip(tabs, QUERIES.keys()):
    with t:
        for item in fetch_news(topic, 6):
