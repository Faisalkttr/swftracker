...
from utils.fred import (
    fred_series,
    fred_status,
    fred_series_with_fallback,
    fred_units,
)
...
# ----------------------------------------------------------------------
# Live charts (only when FRED is reachable)
# ----------------------------------------------------------------------
if ok:
    ...
    with col2:
        ...
        if cust is not None and not cust.empty:
            # Normalize units using FRED's own metadata (millions -> billions)
            ...
            fig3.update_layout(template="plotly_dark", height=320,
                               title=f"Series used: {used_series}",
                               yaxis_title="$B",
                               paper_bgcolor="rgba(0,0,0,0)",
                               plot_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig3, use_container_width=True)

            latest_val = float(cust_display["value"].iloc[-1])
            prev_val = float(cust_display["value"].iloc[-2]) if len(cust_display) > 1 else latest_val
            delta = latest_val - prev_val
            st.metric("Latest reading", f"${latest_val:,.0f}B", f"{delta:+,.0f}B vs prior")
        else:
            st.warning(
                "Foreign Treasury holdings series not available from FRED right now. "
                "This may be a temporary API issue or a retired series ID."
            )

st.divider()

# ----------------------------------------------------------------------
# Fiscal stress map (curated — works even without FRED key)
# ----------------------------------------------------------------------
...
with qcols[2]:
    ...
    st.write("• Are EMs refinancing in USD or local currency?")
